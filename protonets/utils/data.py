...
def loader(opt, splits):
    ...
    for split in splits:
        ...
        if opt['data.dataset'] == 'lsh':
            pdb.set_trace()
            speech_args = filter_opt(opt, 'speech')
            data_dir = os.path.join(os.path.dirname(__file__), '../../data/speech_commands/lsh')
            class_file = os.path.join(os.path.dirname(__file__), '../../data/speech_commands/lsh', split + '.txt')
            ds = FewShotSpeechDataset(data_dir, class_file, n_support, n_query, opt['data.cuda'], speech_args)  
